...
def crop_image(img, area, padding=0):
    ...
    x1 = x - padding
    y1 = y - (2.5*padding)  # extra padding to the top, to see some space above the head
    x2 = x + w + padding
    y2 = y + h + padding
...
def crop_images(input_dir, output_dir):
    ...
    for filename in glob.glob(os.path.join(input_dir, '*.jpg')):
        ...
        try:
            ...
            cropped_image = crop_image(image_rgb, area, padding=30)
        ...
        filename_out = os.path.join(output_dir, '{}-cropped{}'.format(name, ext))
